...
## [FlatList 設定の最適化](https://reactnative.dev/docs/optimizing-flatlist-configuration)

### FlatList Props

[FlatList][] のパフォーマンスを向上に役立つ`Props`を記載。
...
`true`の場合、ビューポートの外側にある View は、ネイティブの View 階層から切り離される。

- メリット

  ビューポートの外側にある View をネイティブのレンダリングや描画のトラバースから除外することで、メインスレッドの処理時間を短縮し、フレームドロップのリスクを低減できる。

- デメリット

  主に iOS で、コンテンツの欠落などのバグが発生する可能性がある。また、View が解放される訳ではなく、切り離されているだけなので、大幅なメモリの節約にはならないことに注意。
...
バッチごとにレンダリングされるアイテム数を制御するもので、スクロールごとにレンダリングされるアイテムの次のチャンクを表す。

- メリット

  大きな数値を設定すると、スクロール時の視覚的な空白部分が少なくなる。

- デメリット

  大きな数値を設定すると、JavaScript の実行時間が長くなる。それによって、他のイベント処理がブロックされる可能性があり、応答性が悪くなる。
...
バッチレンダリング間のミリ秒単位の遅延（アイテムをレンダリングする頻度）を指定する。

- メリット

  `maxToRenderPerBatch`と組み合わせることで、低頻度のバッチで多くのアイテムをレンダリングしたり、高頻度のバッチで少ないアイテムをレンダリングする…といった調整が可能。

- デメリット

  低頻度のバッチでは空白領域が発生したり、高頻度のパッチでは応答性の問題が発生する可能性がある。そのため、慎重に調整する必要がある。
...
最初にレンダリングされるアイテム数を指定する。

- メリット

  サポートデバイスに合った適切なアイテム数を指定することで、初期レンダリングのパフォーマンスが向上する。

- デメリット

  サポートデバイスに対して少ないアイテム数を指定すると、初期レンダリングで空白領域ができてしまう。
...
表示領域外も含めてレンダリングされるアイテムの最大数を、ビューポートの高さの単位で指定する。デフォルト（21）の場合、表示領域に加えて、ビューポートの上下 10 画面分までをレンダリングする。

- メリット

  大きな数値を設定すると、高速スクロールによる空白領域の発生リスクが減る。一方、小さな数値を設定すると、レンダリングするアイテム数が減り、メモリを節約できる。
